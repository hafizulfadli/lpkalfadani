    ...
    <div class="bg-[url({{'/images/hero2.jpg'}})] sm:bg-white object-cover bg-cover bg-center">
        <div class="py-8 sm:py-12 flex flex-col gap-8 md:gap-16">
            <div class="container flex flex-col gap-8 md:gap-16">
                <h1 class="font-semibold text-2xl sm:text-4xl xl:text-5xl text-hijau-unand flex items-center justify-center gap-2 my-36">Berita</h1>
            </div>
        </div>
    </div>
        ...
        <div class="flex flex-col md:flex-row justify-between gap-8">
            <div class="flex flex-col gap-8 w-full md:w-3/5">
                ...
                <div>
                    ...
                    <h2 class="uppercase py-4 text-yellow text-3xl font-semibold">
                        {{ $beritanamakategoris->nama_kategori }}
                    </h2>
                </div>
                        ...
                        <div class="relative h-[900px] my-4 overflow-hidden">
                            <div class="hidden duration-300 ease-in-out" data-carousel-item>
                                <div class="grid grid-cols-2 grid-rows-2 gap-8">
                                    @foreach ($beritakategoris as $data)
                                    <div>
                            ...
                            <div class="hidden duration-300 ease-in-out" data-carousel-item>
                                <div class="grid grid-cols-2 grid-rows-2 gap-8">
                                    @foreach ($beritakategoris2 as $data )
                                    <div>
                                        <img src="{{ url('thumbnail/'.$data->thumbnail) }}" alt="" class="pb-2 w-[441.2px] h-[253.467px]">
                                        <a href="{{ route('detailberita',$data->slug) }}"> <h5 class="font-medium text-lg pb-1">{{ $data->title }}</h5></a>
                                       <p class="text-sm">{{ date('d F Y', strtotime($data->tanggal)) }}</p>
                                        <p class="text-sm">{!! \Illuminate\Support\Str::limit($data->kontent, 150, ' ...') !!}
                                        </p>
                                    </div>
                                    @endforeach
                                </div>
            ...
            <div class="flex flex-col gap-8 w-full md:w-2/5">
                <div>
                   ...
                   <h2 class="uppercase py-4 text-yellow text-3xl font-semibold">{{ $beritanamakategoris->nama_kategori }} Populer</h2>
                </div>
