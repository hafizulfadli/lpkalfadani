    ...
    <div class="container my-28 flex flex-col md:flex-row justify-between gap-8 px-4">
        <div class="flex flex-col rounded-3xl shadow-lg w-full md:w-3/4">
            <div class="flex items-start justify-center flex-col gap-4 p-4">
                ...
                <h1 class="font-medium text-4xl text-almost-black">{{ $kegiatan->judul }}</h1>
                <h5 class="font-normal text-base text-gray-500">Padang, {{ date('d F Y ', strtotime($kegiatan->tanggal)) }}</h5>
            </div>
            <div class="flex flex-col gap-8 p-4 w-full">
        <img src="{{url('thumbnail/',$kegiatan->thumbnail) }}" class="w-full">
                <p class="font-medium text-lg text-almost-black">{!! $kegiatan->konten !!}</p>
            </div>
        </div>
